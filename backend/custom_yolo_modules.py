...
class AAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        # FlashAttention or fallback
        try:
            from flash_attn.flash_attn_interface import flash_attn_func
            USE_FLASH_ATTN = x.is_cuda and torch.cuda.get_device_capability()[0] >= 8
        except Exception:
...
class TryExcept(nn.Module):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__()
        self.module = nn.Sequential(*args) if args else nn.Identity()
...
def register_modules():
    """Register all custom modules with torch"""
    import sys
    current_module = sys.modules[__name__]
    
    # Get all classes that are nn.Module subclasses
    for name in dir(current_module):
        obj = getattr(current_module, name)
        if isinstance(obj, type) and issubclass(obj, nn.Module) and obj != nn.Module:
            # Register the module so it can be found by model loading
            globals()[name] = obj

def patch_ultralytics_modules():
    """Patch ultralytics modules with custom modules"""
    try:
        ...
        for name, module in custom_modules.items():
            setattr(block_module, name, module)
        ...
        print(f"⚠️  Could not patch ultralytics modules: {e}")

# Auto-register when imported
register_modules()
patch_ultralytics_modules()

print("🔧 Custom YOLO modules registered:")
print("   - C3k, C3k2, Bottleneck, TryExcept (YOLOv11/v12)")
